	...
	return func(c *gin.Context) {
		...
		go func() {
			...
			if err := query.Exec(); err != nil {
				log.Println("Ошибка при добавлении сообщения в базу данных:", err)
			}
		}()
...
func SaveImage(session *gocql.Session) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		go func() {
			...
			if err := query.Exec(); err != nil {
				log.Println("Ошибка при добавлении изображения в базу данных", err)
			}
		}()
...
func GetLastMessages(session *gocql.Session, lastID int64) ([]ChatMessage, int64, error) {
	var newLastID int64
	query := "SELECT username, message, created_at, image, audio_data FROM messages WHERE chat_id = 1 AND created_at < ? ORDER BY created_at DESC LIMIT 5 ALLOW FILTERING"

	iter := session.Query(query, lastID).Iter()
	defer iter.Close()
	...
	for {
		...
		if imageUrl != "" {
			...
			if err == nil {
				defer object.Close()
				if imageData, err := io.ReadAll(object); err == nil {
		...
		if audioUrl != "" {
			...
			if err == nil {
				defer object.Close()
				if audioData, err := io.ReadAll(object); err == nil {
					msg.Audio = "data:audio/wav;base64," + base64.StdEncoding.EncodeToString(audioData)
				}
			}
		}

		if newLastID == 0 || msg.CreatedAt < newLastID {
			newLastID = msg.CreatedAt
		}
